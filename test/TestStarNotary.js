...
it('lets user2 buy a star and decreases its balance in ether', async() => {
    ...
    assert.notEqual(value, Number(starPrice));
}).timeout(10000);
...
it('can add the star name and star symbol properly', async() => {
    ...
    await instance.createStar('Hey Dom', 10, {from: user});

    assert.equal(await instance.name(), 'Star Dom');
    assert.equal(await instance.symbol(), 'SDM');
}).timeout(10000);
...
it('lets a user transfer a star', async() => {
    ...
    await instance.createStar("Hello 1111!", tokenId, {from: user1});
